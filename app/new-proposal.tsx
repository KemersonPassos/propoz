...
export default function NewProposal() {
  ...
  const exportProposalToPDF = async (proposalData: any, profileData: any) => {
    try {
      setLoading(true);
      const htmlTemplate = generateProposalPdfHtml(proposalData, profileData);
          ...
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
            <View style={{ backgroundColor: '#fff', borderRadius: 20, padding: 24, alignItems: 'center' }}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  const shareUrl = `https://propoz.pages.dev/view/${savedProposalData?.share_id || savedProposalData?.id}`;
                  await Share.share({
